...
export default function Navbar() {
  ...
  return (
    ...
    >
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex md:hidden items-center space-x-4">
            <Link href="/cart" className="relative p-2 text-slate-300">
              <ShoppingCart className="w-5 h-5" />
              {cartCountMounted > 0 && (
                <span className="absolute top-0 right-0 bg-brand-orange text-white text-[9px] font-bold rounded-full w-4 h-4 flex items-center justify-center">
                  {cartCountMounted}
                </span>
              )}
            </Link>
      ...
      {/* Mobile Menu Panel */}
      {mobileMenuOpen && (
        <div className="md:hidden bg-brand-navy border-b border-brand-slate/40 animate-fade-in-up">
          <div className="px-2 pt-2 pb-4 space-y-1 sm:px-3">
                ...
                <Link
                  ...
                  className={`block px-3 py-2.5 rounded-md text-base font-medium ${
                    isActive
                      ...
                      : "text-slate-300 hover:bg-brand-slate/20 hover:text-white"
                  }`}
                >
                  {link.name}
                </Link>
            ...
            {session?.user?.role === "ADMIN" || session?.user?.role === "SUPER_ADMIN" ? (
              <Link
                href="/admin/dashboard"
                className="block px-3 py-2.5 rounded-md text-base font-medium text-brand-orange bg-brand-orange/5 border-l-4 border-brand-orange"
              >
                Admin Dashboard
              </Link>
            ) : null}
            ...
            <div className="border-t border-slate-800 my-2 pt-2 px-3">
              {session ? (
                <div className="flex items-center justify-between">
                  ...
                  <button
                    onClick={() => signOut({ callbackUrl: "/" })}
                    className="text-xs px-3 py-1.5 bg-red-950/40 text-red-400 border border-red-900/60 rounded-md"
                  >
                    Logout
                  </button>
                </div>
                ...
                <div className="flex flex-col space-y-2">
                  <Link
                    href="/login"
                    className="block text-center w-full px-4 py-2 border border-slate-700 rounded-md text-slate-300 text-sm"
                  >
                    Customer Sign In
                  </Link>
                  <Link
                    href="/admin-login"
                    className="block text-center w-full px-4 py-2 bg-brand-orange/15 border border-brand-orange/30 rounded-md text-brand-orange text-sm font-semibold"
                  >
                    Admin Portal
                  </Link>
                </div>
